...
class HMSController extends ComController
{
    ...
    public function geninputs($sheetid)
    {
        ...
        for ($i = 0; $i < 30; $i++) {
            ...
            $content = str_replace("t_part6", "forcastm", $content);
    ...
    public function feditmodel($sheetid, $project)
    {
        ...
        $gagecontent = preg_replace("#/780828/#m", "/forcastm/", $gagecontent);
    ...
    public function retrievepcpDataf()
    {
        ...
        foreach ($guages as $guage) {
            //780828_pcp_1.txt
            $filename = C('modeloutput') . "hec/forcastm/" . $project . "_pcp_" . $guage . "_" . $id . ".txt";
            $filedata = file_get_contents($filename);
            $filedata = str_replace("'", "\"", $filedata);
            $r = json_decode($filedata);
            if ($type == 1) {
    ...
    public function retrieveflowDataf()
    {
        ...
        foreach ($guages as $guage) {
            //780828_pcp_1.txt
            foreach ($rs3 as $item) {
                $filename = C('modeloutput') . "hec/forcastm/" . $project . "_sim_" . $guage . "_" . $item["sheetid"] . ".txt";
                $filedata = file_get_contents($filename);
                $filedata = str_replace("'", "\"", $filedata);
                $r = json_decode($filedata);
                if ($type == 1) {
    ...
    function genalldata()
    {
        ...
        $idarray = array();
        foreach ($rs2 as $item) {
            $pcppathname = $item["name"] . $rs["ipcppathname"];
            $patharray[] = $pcppathname;
            $idarray[] = $item["id"];
        }

        $this->commongendata($type, $file, $patharray, $idarray, $timewindow, $project);
        ...
        $idarray = array();
        foreach ($rs3 as $item) {
            $pcppathname = $item["name"] . $rs["iflowpathname"];
            $patharray[] = $pcppathname;
            $idarray[] = $item["id"];
        }
        $this->commongendata($type, $file, $patharray, $idarray, $timewindow, $project);
        ...
        $idarray = array();
        foreach ($rs3 as $item) {
            $pcppathname = $item["oname"] . $rs["opathname"];
            $patharray[] = $pcppathname;
            $idarray[] = $item["id"];
        }
        $this->commongendata($type, $file, $patharray, $idarray, $timewindow, $project);
    ...
    function genallforcastdata($sheetid)
    {
        ...
        foreach ($rs2 as $item) {
            ...
            $pcppathname = "/XIXI/" . $nameparts[2] . "/PRECIP-INC/" . $tokens[0] . " - " . $tokens[2] . "/1HOUR/FORCASTM/";
            $patharray[] = $pcppathname;
            $idarray[] = $item["id"];
        }
    ...
    function commongendata($type, $file, $patharray, $idarray, $timewindow, $project)
    {
        ...
        foreach ($patharray as $item) {
            $tempstr = $repeat;
            $pcppathname = $item;
            $filename = $project . "_" . $type . "_" . $idarray[$index] . ".txt";
            $tempstr = str_replace("outputpath", $pcppathname, $tempstr) . "\r\n";
            $outputfile = C('modeloutput') . "hec/" . $filename;
    ...
    function commongendataf($type, $file, $patharray, $idarray, $timewindow, $project, $sheetid)
    {
        ...
        foreach ($patharray as $item) {
            ...
            $tempstr = str_replace("outputpath", $pcppathname, $tempstr) . "\r\n";
            $outputfile = C('modeloutput') . "hec/forcastm/" . $filename;
            $body = $body . str_replace("outputtxtfile", $outputfile, $tempstr) . "\r\n";
            $index++;
        }
        ...
        $str = C('hecsoftroot') . "\r\n";
        $str = $str . "cd " . C('dsssoft') . "\r\n";
        $str = $str . "HEC-DSSVue.cmd " . C('hecmodel') . "/runtime/" . $project . "_output.py\r\n";
        file_put_contents(C('hecmodel') . "/runtime/" . $project . "_output.bat", $str);


        file_put_contents(C('hecmodel') . "/runtime/" . $project . "_output.py", $content);
        $command = C('hecmodel') . "/runtime/" . $project . "_output.bat" . " >>E:/php/Apache24/htdocs/wms/Public/hecmodel/runtime/" . $project . "_output.log.txt";
        // echo $command;
        exec($command, $rs, $st);
